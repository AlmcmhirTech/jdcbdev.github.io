...
class Account{
    ...
    public function fetchAccounts() {
        $sql = "SELECT first_name, last_name, username, role FROM account"; // SQL query to fetch accounts
    
        $query = $this->db->connect()->prepare($sql); // Prepare the SQL statement
    
        $data = []; // Initialize an array to hold the fetched data
    
        if ($query->execute()) {
            $data = $query->fetchAll(PDO::FETCH_ASSOC); // Fetch all rows from the result set
        }
